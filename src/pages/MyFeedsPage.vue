<template>
  <div :class="['page-container', { 'is-embedded': embedded }]" class="custom-scrollbar" @scroll="handleScroll">
    <div v-if="!embedded" class="page-header">
      <div class="header-main">
        <div class="header-titles">
          <h2 class="page-title"><i class="fas fa-stream icon"></i> 我的动态</h2>
          <span class="page-subtitle">同步酷安账号发布过的动态</span>
        </div>
    ...
    <div v-if="!authStore.isLoggedIn" class="empty-wrapper">
      <EmptyState title="登录后查看我的动态" description="登录酷安账号后，此处会读取真实的动态记录" />
    ...
    <template v-else>
      ...
      <div v-else class="feed-list">
        ...
        <div class="pagination-footer">
          ...
          <div v-else-if="noMore" class="no-more">已加载全部动态记录</div>
        </div>
      </div>
    </template>
  </div>
...
async function load(refresh = false) {
  ...
  try {
    const res = await CoolapkTauriAPI.getUserFeeds(String(authStore.user.uid), page.value, 'feed');
