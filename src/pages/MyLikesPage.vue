    ...
    <div v-if="!embedded" class="page-header">
      <div class="header-main">
        <div class="header-titles">
          <h2 class="page-title"><i class="far fa-thumbs-up icon"></i> 我的赞</h2>
          <span class="page-subtitle">同步酷安账号点赞过的动态</span>
        </div>
        <AppButton v-if="authStore.isLoggedIn && !embedded" variant="secondary" size="sm" icon="fas fa-sync-alt" :loading="loading" @click="load(true)">刷新</AppButton>
      </div>
    </div>

    <div v-if="!authStore.isLoggedIn" class="empty-wrapper">
      <EmptyState title="登录后查看我的赞" description="登录酷安账号后，此处会读取真实的点赞记录" />
      <AppButton variant="primary" size="sm" @click="authStore.openLoginModal()">立即登录</AppButton>
    </div>
    <template v-else>
      <LoadingState v-if="loading && items.length === 0" text="正在读取点赞记录..." />
      <ErrorState v-else-if="error && items.length === 0" title="加载点赞记录失败" :message="error" @retry="load(true)" />
      <EmptyState v-else-if="!items.length" title="暂无点赞记录" description="你点赞过的动态会出现在这里" />
      <div v-else class="feed-list">
        <FeedCard v-for="item in items" :key="item.id || item.entityId" :feed="item" @deleted="removeItem" />
        <div class="pagination-footer">
          <LoadingState v-if="loadingMore" text="正在加载更多..." />
          <div v-else-if="noMore" class="no-more">已加载全部点赞记录</div>
        </div>
      </div>
    </template>
  </div>
...
async function load(refresh = false) {
  ...
  try {
    const res = await CoolapkTauriAPI.getUserLikeList(String(authStore.user.uid), page.value);
